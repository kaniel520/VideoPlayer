...
//        formatCtx->duration = 100000;
//        formatCtx->max_delay = 1000;
    }
    int err_code = 0;
    formatCtx = NULL;
    if ((err_code = avformat_open_input(&formatCtx, [path cStringUsingEncoding: NSUTF8StringEncoding], NULL, &avdic)) < 0) {
        if (formatCtx)
    ...
    /// 缩短延时
//    formatCtx->max_analyze_duration2 = 1000;
//    formatCtx->probesize = 2048;
    // 配置延时参数
    if (self.parameter) {
        id val = [self.parameter valueForKey: FYParameterDecoderMaxAnalyzeDuration];
        if ([val isKindOfClass:[NSNumber class]])
            formatCtx->max_analyze_duration2 = [val floatValue];
        
        val = [self.parameter valueForKey: FYParameterDecodeerProbesize];
        if ([val isKindOfClass:[NSNumber class]])
            formatCtx->probesize = [val floatValue];
    }
...
{    
    ...
    // -- beigin 针对播放 rtsp做了如下配置--------------------------------
    
    if ([_path hasPrefix:@"rtsp"]) {
        
        codecCtx->time_base.num = 1;
        codecCtx->frame_number = 8; //每包一个视频帧
...
//        NSLog(@"begin read frame+++++");
        if (av_read_frame(_formatCtx, &packet) < 0) {
            /// 读取失败或者结束
            _isEOF = YES;
            break;
        }
//        NSLog(@"end read frame======");
